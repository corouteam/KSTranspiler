...
fun SwiftParser.TypeContext.toAst(considerPosition: Boolean = false) : Type = when (this) {
    ...
    is SwiftParser.ContentModeTypeContext -> ContentModeType(toPosition(considerPosition))
    is SwiftParser.ImageWidgetContext -> ImageComposableType()
    is SwiftParser.TextWidgetContext -> TextComposableType(toPosition(considerPosition))
    is SwiftParser.SwiftUIColumnParamContext -> ColumnComposableType(toPosition(considerPosition))
    is SwiftParser.SpacerWidgetContext -> SpacerComposableType(toPosition(considerPosition))
    is SwiftParser.ZStackWidgetContext -> ZStackComposableType(toPosition(considerPosition))
    is SwiftParser.HStackWidgetContext -> HStackComposableType(toPosition(considerPosition))
    is SwiftParser.ButtonWidgetContext -> ButtonComposableType(toPosition(considerPosition))
    is SwiftParser.DividerWidgetContext -> DividerComposableType(toPosition(considerPosition))
    is SwiftParser.HorizontalAlignmentContext -> HorizontalAlignmentType(toPosition(considerPosition))
    is SwiftParser.VerticalAlignmentContext -> VerticalAlignmentType(toPosition(considerPosition))
    is SwiftParser.AspectRatioSuffixContext -> AspectRatioType(toPosition(considerPosition))
